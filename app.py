...
# # BMI Calculator Page
# # elif nav == '🏋️‍♀️ BMI Calculator':
# #     st.header("🏋️‍♀️ BMI Calculation & Suggested Sports")
...
# #                 # Determine sport recommendation based on BMI
# #                 if bmi < 18.5:
...
# #                 # Save athlete info to session state
# #                 st.session_state.bmi_athlete = {
# #                     'name': name,
# #                     'age': age,
# #                     'height': height,
# #                     'weight': weight,
...
# #                 # Display BMI result and sport suggestion
# #                 st.markdown(f"""
...
# #                             <tr><td><strong>BMI:</strong></td><td>{round(bmi, 2)}</td></tr>
# #                             <tr><td><strong>Suggested Sport(s):</strong></td><td>{sport}</td></tr>
# #                         </table>
# #                     </div>
# #                 """, unsafe_allow_html=True)

# #                 # Save the BMI data to CSV
...
# #                 st.download_button(
# #                     "💾 Download BMI Data (CSV)",
# #                     bmi_data.to_csv(index=False),
# #                     f"bmi_data_{name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
# #                     "text/csv"
# #                 )
...
#                     # Update status
#             status_text.text("🟢 Baseline measurement complete. Now monitoring in real-time.")

#         # Reading phase (10 seconds)
#             readings = []
#             while time.time() < baseline_start + total_duration:
#             # Read serial data for readings
#                 if ser.in_waiting:
#                     line = ser.readline().decode('utf-8').strip()
#                     raw_data_list.append(line)
#                     st.session_state.biometrics['raw_data'].append(line)

#                 # Process readings for real-time
#                 if line.startswith("GSR="):
#                     try:
#                         gsr = int(line.split('=')[1].split()[0])
#                         readings.append({'gsr': gsr, 'pulse': baseline_pulse, 'oxygen': baseline_oxygen})
#                     except:
#                         continue
...
#                 # Display biometrics metrics
#                 st.session_state.biometrics['gsr'].append(np.mean(baseline_values['gsr']))  # Appending for chart later
...
# BMI Calculator Page
    ...
    with st.expander("🧑 User Information", expanded=True):
        with st.form("bmi_form"):
            cols = st.columns(2)
            with cols[0]:
                name = st.text_input("Full Name", placeholder="e.g., Simone Biles", key="bmi_name")
    ...
    if 'bmi_data' in st.session_state:
        ...
        <div class="metric-card">
            ...
            <table>
                ...
                <tr><td><strong>Recommended Sports:</strong></td><td>{", ".join(recommended_sports)}</td></tr>
            </table>
        </div>
        """, unsafe_allow_html=True)
        
        # Add BMI chart reference
        st.markdown("""
        <div class="metric-card">
        ...
        st.download_button(
            ...
            file_name=f"bmi_data_{user['name']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv"
        )
